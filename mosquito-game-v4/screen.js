...
function clickBtnScreen(event){
	if(event.offsetX>75 && event.offsetX<320 &&
		event.offsetY>250 && event.offsetY<290 ){
		game.removeEventListener("mousemove", overBtnScreen, false); // Succeeds
		game.removeEventListener("click", clickBtnScreen, false); // Succeeds
		game.style.cursor = "default";
		func = running
	}
}
function running(){
	ctx.drawImage(backgroundImg, 0,0,400,400);
		ctx.drawImage(mosquitoImg, mosquito.x-mosquito.rad, mosquito.y-mosquito.rad, mosquito.rad*2, mosquito.rad*2);
		game.addEventListener('click', killMosquitoes)
}
...
function setup(){
	game.addEventListener('mousemove', overBtnScreen)
	game.addEventListener('click', clickBtnScreen)
	cronometerStart()
}

setup()
func = loopStartScreen
